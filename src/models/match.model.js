...
const UserModel = require("./user.model"); //! Potential coupling when we want to move to microservices - used once
...
class MatchModel {
  ...
  async _storeMatchedUsers(matchData) {
    //* Tested & Working
    let batch = db.batch();
  ...
  // Store the date of the last matched user
  _storeLastMatchDate(matchData) {
    if (!matchData.length) return;
    // Use first user found to get the iniator
    let firstMatch = matchData[0];

    let updateData = {
      lastMatchDate: FieldValue.serverTimestamp()
    };

    return UserModel.updateUser(firstMatch.initiatorUser.id, updateData);
  }

  /** Store details of successful match
   * @public
   * @param {Array<Object>} matchData An array of match data on users involved in the match
   * @return {Promise<Firebase:WriteResult()>} Firebase batch commit result
   */
  async addMatch(matchData) {
    //* Tested & Working
    this._storeMatchedUsers(matchData);
    this._storeLastMatchDate(matchData);

    // Store last match date

...
module.exports.ajInterestMatchCalc = (user1Interests, user2Interests) => { //? Compares 2 user interests and determines a match score
    ...
    let matchScore = matchedInterests.reduce((prev, currInterest) => { //? CurrInterest will contain weight information later on
        ...
        let currInterestPoints = (user1InterestWeight + user2InterestWeight)/NUM_PEOPLE_COMPARED;
        let aggregatePoints = prev + currInterestPoints; //? Total points so far
        return aggregatePoints;
    },0).toFixed(5); //? 5 decimal places with max of 1. 3 decimal places as percentage ~ enough variance to sort matches well
